<div class="box box-solid box-clean flat no-shadow">
      <a data-toggle="collapse" data-parent="#accordion" href="#collapseJSON">
      <div class="box-header with-border">
          <h3 class="box-title">JSON message</h3>
          <div class="box-tools pull-right">
            <button class="btn btn-box-tool" ><i class="fa fa-minus"></i></button>
          </div>
      </div>
      </a>
      
      <div id="collapseJSON" class="panel-collapse collapse">
      <div class="box-body" id="msgJSON">
      </div>
      <div class="box-footer" id="footerJSON">
          <div class="btn btn-success btn-sm center-block flat" id="btnJSON" data-toggle="tooltip" title="copy to clipboard" onclick="copyToClipboard('result')" style="width: 100px;">
              <i class="fa fa-copy"></i> copy json
          </div>
      </div>
      </div>
</div>