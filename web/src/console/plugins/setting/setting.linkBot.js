function showBots(str,option){
    ...
    for (var x in purchasedBots) {
        ...
        if ( (str!=" ") && ( (str.length==0) || (bot['name'].toLowerCase()).indexOf(str.toLowerCase())!=-1 ) )  {
            ...
            wHTML += ('<img class="card-icon" src="data:image/jpeg;base64,' + bot['imagePath'] + '" onClick=openSingleBot(this,"' + option + '","' + bot['botId'] + '",'+($.inArray(bot['botId'], purchasedBots) != -1)+'); ><i class="'+ bot['imagePath']+'"></i></img>');
