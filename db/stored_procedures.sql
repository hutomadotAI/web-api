...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`userTableReader`@`127.0.0.1` PROCEDURE `getUserId`(IN `nameOfUser` VARCHAR(50) CHARSET utf8mb4, IN `checkEmail` BOOLEAN)
    READS SQL DATA
BEGIN
    IF checkEmail THEN
      SELECT id
...
CREATE DEFINER=`rateLimiter`@`127.0.0.1` PROCEDURE `rateLimitCheck`(
  IN `in_dev_id` VARCHAR(50) CHARSET utf8mb4,
  IN `in_rate_key` VARCHAR(50) CHARSET utf8mb4,
