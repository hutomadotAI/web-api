...
ALTER TABLE `intent`
  ADD COLUMN `reset_context_on_exit` TINYINT(1) NOT NULL DEFAULT '0' AFTER `conditions_default_response`;
...
BEGIN
    INSERT INTO `intent` (`aiid`, `name`, `topic_in`, `topic_out`, `context_in`, `context_out`, `conditions_in`,
      `conditions_default_response`, `reset_context_on_exit`)
      SELECT `aiid`, `in_name`, `in_topic_in`, `in_topic_out`, `in_context_in`, `in_context_out`, `in_confitions_in`,
      `in_conditions_default_response`, `in_reset_context_on_exit`
      FROM ai
